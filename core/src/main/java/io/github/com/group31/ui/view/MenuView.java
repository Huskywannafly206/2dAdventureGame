...
import io.github.com.group31.ui.model.MenuViewModel;
...
public class MenuView extends View<MenuViewModel> {
    ...
    public MenuView(Stage stage, Skin skin, MenuViewModel viewModel) {
        ...
        String defaultOption = viewModel.hasSaveFile() ? MenuOption.CONTINUE_GAME.name() : MenuOption.START_GAME.name();
        this.selectedItem = findActor(defaultOption);
...
//        add(label).padRight(5.0f).padBottom(5f).expand().align(Align.bottomRight);
    ...
    private void setupMenuContent() {
        ...
        if (viewModel.hasSaveFile()) {
            TextButton continueBtn = new TextButton("Continue", skin);
            continueBtn.setName(MenuOption.CONTINUE_GAME.name());
            onClick(continueBtn, viewModel::continueGame);
            onEnter(continueBtn, this::selectMenuItem);
            contentTable.add(continueBtn).row();
        }
        ...
        contentTable.add(textButton).padTop(viewModel.hasSaveFile() ? 10f : 0f).row();
    ...
    public void onSelect() {
        MenuOption menuOption = MenuOption.valueOf(this.selectedItem.getName());
        switch (menuOption) {
            case CONTINUE_GAME -> viewModel.continueGame();
